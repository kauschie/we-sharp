...
from audiolm_pytorch.utils import AudioConditionerBase, write_progress, _start_times
    ...
    def generate(
        ...
        **kwargs
        ...
        if exists(prime_wave):
            print(f"[DEBUG] Before wav2vec.forward() - prime_wave shape: {prime_wave.shape}") 
        ...
        for ind in tqdm(range(start_length, max_length), desc = 'generating semantic'):
            ...
            if track_progress and ind % 10 == 0:
                percent = ((ind + 1) / max_length) * 100
                length_seconds = (ind + 1) / 50.0
                write_progress("semantic", percent, length_seconds, track_progress)
    ...
    def generate(
        ...
        **kwargs
        ...
        if max_time_steps is None:
            max_time_steps = semantic_token_ids.shape[1]
        ...
        for time_step in tqdm(range(init_coarse_time_step, max_time_steps), desc = 'generating coarse'):
            for ind in range(self.num_coarse_quantizers):
                ...
                last_coarse_logits[:, self.coarse_eos_id] = float('-inf')
            ...
            if track_progress and time_step % 10 == 0:
                percent = (time_step + 1) / max_time_steps * 100
                length_seconds = (time_step + 1) / 75.0
                write_progress("coarse", percent, length_seconds, track_progress)
...
class FineTransformerWrapper(nn.Module):
    ...
    def generate(
        ...
        **kwargs
        ...
        print(f"[DEBUG] coarse_token_ids shape: {coarse_token_ids.shape}")
        coarse_token_ids = rearrange(coarse_token_ids, 'b ... -> b (...)')
        print(f"[DEBUG] coarse_token_ids shape after rearrange: {coarse_token_ids.shape}")

        batch, device = coarse_token_ids.shape[0], self.device

        print(f"[DEBUG] it's grabbing batch which is: {batch} on device {device}")
        ...
        for time_step in tqdm(range(init_fine_time_step, max_time_steps), desc = 'generating fine'):
            for ind in range(self.num_fine_quantizers):
                ...
                if self.eos_id < last_fine_logits.shape[-1]:
                    last_fine_logits[:, self.eos_id] = float('-inf')
            ...
            if track_progress and time_step % 10 == 0:
                percent = (time_step + 1) / max_time_steps * 100
                length_seconds = (time_step + 1) / 75.0
                write_progress("fine", percent, length_seconds, track_progress)
        ...
        print(f"[DEBUG] final shape before decode: {coarse_and_fine_ids.shape}")
        ...
        #     print(f"[DEBUG] Decoded wav shape before split: {wav.shape}")  # <-- ADD THI
...
class AudioLM(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # If desired_duration is set but no semantic max_length, calculate it
        if desired_duration is not None and max_length is None:
            semantic_frame_rate = 50
            max_length = int(desired_duration * semantic_frame_rate)

        if track_progress:
            _start_times.clear()
            write_progress("semantic", 0, 0, track_progress)
        ...
        if track_progress:
            write_progress("semantic", 100, len(semantic_token_ids[0]) / 50.0, track_progress)

        if return_semantic:
            return semantic
        print("Semantic token shape:", semantic_token_ids.shape)

        codec_sr = 24000
        hop_len = 2 * 4 * 5 * 8 # hard coded stride product = 320
        frame_rate = codec_sr/hop_len
        max_time_steps = int(desired_duration*frame_rate) if desired_duration else semantic_token_ids.shape[1]


        if track_progress:
            _start_times.clear()
            write_progress("coarse", 0, 0, track_progress)
        ...
        print("Coarse token shape:", coarse_token_ids_or_recon_wave.shape)
        
        if track_progress:
            write_progress("coarse", 100, len(coarse_token_ids_or_recon_wave[0]) / 50.0, track_progress)
        ...
        if track_progress:
            _start_times.clear()
            write_progress("fine", 0, 0, track_progress)
        ...
        if track_progress:
            write_progress("fine", 100, len(generated_wave[0])/24000.0, track_progress)


        print(f"[DEBUG] type returned from fine.generate: {type(generated_wave)}")
        if isinstance(generated_wave, torch.Tensor):
            print(f"[DEBUG] Shape of generated_wave tensor: {generated_wave.shape}")
        elif isinstance(generated_wave, list):
            print(f"[DEBUG] Length of generated_wave list: {len(generated_wave)}")
            for i, t in enumerate(generated_wave):
                print(f"  [DEBUG] generated_wave[{i}] shape: {t.shape}")
