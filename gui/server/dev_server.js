...
// CORS setup
app.use(cors({
  origin: `https://136.168.201.107:${CLIENT_PORT}`, // Allow specific origin
...
app.use('/audio/uploaded', (req, res, next) => {
  res.setHeader('Access-Control-Allow-Origin', `https://136.168.201.107:${CLIENT_PORT}`)
...
app.post("/generate", async (req, res) => {
  ...
  console.log("Running gen_audio_batch2.py with input:", inputPath, "and output path:", outputPath)
...
https.createServer(options, app).listen(SERVER_PORT, () => {
  ...
  console.log(`Server URL: https://136.168.201.107:${SERVER_PORT}`)

  // Send reminder to check if client port is same as here
  console.log(`Assuming that the CLIENT PORT is: ${CLIENT_PORT}`)
})