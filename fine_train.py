...
def log_fn(logs):
    ...
    if fine_trainer.is_main and (steps > 0) and (steps % validation_interval) == 0:  # Example condition for validation
        valid_loss = 0
        unwrapped_model = fine_trainer.accelerator.unwrap_model(fine_trainer.train_wrapper)
        
        for _ in range(fine_trainer.average_valid_loss_over_grad_accum_every):
            data_kwargs = fine_trainer.data_tuple_to_kwargs(next(fine_trainer.valid_dl_iter))
        ...
        valid_loss /= fine_trainer.average_valid_loss_over_grad_accum_every

        fine_trainer.print(f'Step {steps}: valid loss {valid_loss}')
        logger.info(f'Step {steps}: valid loss {valid_loss}')
        writer.add_scalar("Validation Loss", valid_loss, steps) # save to tensorboard
        fine_trainer.accelerator.log({"valid_loss": valid_loss}, step=steps)

    if fine_trainer.is_main and (steps > 0) and (steps % model_save_interval) == 0:
        model_path = str(fine_trainer.results_folder / f'fine.transformer.{steps}.interval.pt')
        fine_trainer.save(model_path)
        fine_trainer.print(f'{steps}: saved model to {str(fine_trainer.results_folder)}')
